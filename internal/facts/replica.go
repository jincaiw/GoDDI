...
func (o *ObservationOutbox) ReadReplicaPageTx(ctx context.Context, tx *sql.Tx, after int64, limit int) (ReplicaPage, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT event_id, version, entity, action, generation,
		sequence, source, occurred_at, payload_version, payload, attempts,
		next_attempt_at, last_error, status
		FROM dhcp_ipam_observation_events WHERE sequence > ? ORDER BY sequence ASC LIMIT ?`, after, limit)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.Envelope.EventID, &item.Envelope.Version,
			...
			&nextAttempt, &item.LastError, &status); err != nil {
