...
func (m *Manager) CreateReservation(scopeID, ip, mac, hostname string, opts ReservationOptions) (*Reservation, error) {
	if ip == "" || mac == "" {
		return nil, fmt.Errorf("ip_address and mac_address are required")
	}
			...
			if subnet != "" {
				...
				if perr == nil {
					...
					if parsed == nil || !ipNet.Contains(parsed) {
						return nil, fmt.Errorf("IP %s is not within scope subnet %s", ip, subnet)
					}
				}
			}
			if startIP != "" && endIP != "" {
				...
				if parsed != nil && lo != nil && hi != nil && !ipInRangeInclusive(parsed, lo, hi) {
					return nil, fmt.Errorf("IP %s is outside scope range %s..%s", ip, startIP, endIP)
				}
			}
		}
	...
	if err != nil {
		...
		if isUniqueConstraintErr(err, "mac_address") {
			return nil, fmt.Errorf("MAC address %s already has a reservation", mac)
		}
