...
- 对所有页面中的表单开启、确认框开启、行操作与内联回调继续扫描后，为 DNS zone/record/DNSSEC/forwarder/blocklist、DHCP scope/reservation/option 创建、租约释放、allowed-list 导入、备份恢复/删除和配置回滚打开动作补齐 `read`/`write`/`delete` 前置检查；用户开关和用户/组/角色/IPAM/DHCP 表格内联编辑也在回调内重新校验。只读会话管理仍由 API 限定用户只能列出/撤销自身会话，管理员撤销他人会话要求 `user:delete`。
