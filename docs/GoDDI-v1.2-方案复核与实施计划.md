...
- 已新增 Playwright 最小权限登录场景：通过管理员 API 创建仅有 `dns:read` 的临时角色和用户，验证其能打开 DNS 区域页但看不到创建入口、删除动作不可用；用例在 `finally` 中清理临时用户、角色和区域。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。修复后仍待 CI 全量验证，其他模块仍需补充权限矩阵场景。
