...
| W13 产品体验 | IP 地址详情抽屉、子网导入与建池向导、DNS CSV/BIND 预检导入均已有实现；建池冲突服务端闸门和跨模块详情权限隔离已接通；DNS、DHCP、IPAM、RBAC、备份、系统配置和 token 页面操作入口及处理器已完成静态权限逐项核对；只读角色浏览器回归覆盖 DNS、DHCP、IPAM | 最新 CI 需通过；RBAC、备份、系统配置和 token 的端到端权限矩阵仍需补充 |
...
- Playwright 最小权限登录场景通过管理员 API 创建临时只读角色和用户，验证 DNS、DHCP、IPAM 页面可读且创建操作入口隐藏，并验证 DNS 区域行删除操作禁用；用例在 `finally` 中清理临时用户、角色和区域。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，DNS 场景本地通过。之后扩展为三模块覆盖，本地真实浏览器复验通过；RBAC、备份、系统配置和 token 的角色矩阵及本轮最新提交 CI 仍待验收。
