...
> **发布后状态（2026-09-24）**：原计划记录的是实施过程中的阶段状态。PR #1 与 PR #2 已合并到 `main`；正式版本 `v0.23.0` 已发布，发布工作流全量 Go 测试、Linux amd64/arm64 构建及校验和产物均成功。PR #2 补上了 DDNS 复制应用后的 DNS consumer 唤醒，端到端延迟回归连续 5 次通过。`v0.23.0` 是代码交付，不代表现场验收或 HA GA。W01—W14 仍有真实 secondary 互操作、跨主机 fencing/分区、备份密钥与介质恢复、监控触发、逐节点升级和固定硬件容量/长稳等环境依赖验收项；详见各工作包退出条件及发布说明。
...
v1.1 不宜原样作为当前版本的实施清单。它的代码证据锁定在 `d5b5fe3 / v0.5.2`。本次复核起点为当时最新已发布基线 `v0.22.0 / 29e0e6c`；后续实施分支完成 W01/W04 等代码工作，并经 PR #1/#2 合并，最终以 `v0.23.0` 发布。代码发布不等于生产验收完成。
...
实施期间的实现分支曾包含尚未合入的事实事件投递与消费边界改动；现相关代码已随 PR #1/#2 合入并发布。事件顺序、缺口检测、失败事件阻塞、重复投递、消费水位事务、进程重启及数据面 readiness 均需结合各自故障证据判断，不能只因增加了表、接口或测试就关闭 W04/W06/W11。
...
| DHCP→IPAM 持久事件 | 非 HA 默认路径已写入 durable facts outbox 并由 control inbox 原子投影；HA primary 使用 lease/facts 双水位，协议 v3 传输增量 facts，备端校验后事务应用；离线重连、断点 staging 清理、apply 失败回滚、control 写失败恢复、主备重启和接管后首事件连续性均有回归 | 仓内主要实现及本地故障用例已接通；PR #1/#2 CI 通过且已发布于 `v0.23.0`；部署/现场验收未完成 | control DB 长期故障重放、旧主回归对账、端到端对账及真实双节点运行态恢复 |
...
| W04 DHCP→IPAM facts | 非 HA 默认 producer/consumer 与跨库幂等投影已接通；HA protocol v3 已接入运行期 facts delta、连续序列校验、备端事务应用和双水位 ACK gate；自动化覆盖离线重连、主备同时重启、失效 staging 清理、apply 失败回滚、control 写失败恢复及接管后首个 facts 序列连续性 | control DB 长时间不可用/恢复、旧主回归对账和真实网络演练 |
...
| W13 产品体验 | IP 地址详情抽屉、子网导入与建池向导、DNS CSV/BIND 预检导入均已有实现；建池冲突服务端闸门和跨模块详情权限隔离已接通；DNS、DHCP、IPAM、RBAC、备份、系统配置和 token 页面操作入口及处理器已完成静态权限逐项核对；只读 UI 场景覆盖九类资源页面、关键 RBAC 按钮及九类写入拒绝；API 浏览器回归覆盖九类单资源读取隔离、双角色权限并集和受限管理员的 user/role/group 授权工作流；本地真实服务用例通过，PR #1/#2 远端 CI 全通过并已随 `v0.23.0` 发布 | 授权写入流程覆盖 API 工作流，未覆盖每个管理对话框的完整鼠标操作路径及目标部署验收 |
| W14 容量认证 | 当前无固定硬件/混合负载容量结论 | 固定硬件、负载、故障矩阵和长稳实测；未测数字不对外承诺 |

W12-a/W12-c 隔离式 restore 演练通过；W13 真实浏览器/API 权限场景在本地通过。实施阶段 `fc5ebff` 的本地全量 Go tests、race、vet、golangci-lint 与两组远端 test/lint/vulnerability CI 曾全部通过；其后的 PR #2 又修复 DDNS 应用后 consumer 的唤醒时延，并由合并提交 `069522d` 对应的 PR CI 及正式发布工作流验证。`v0.23.0` 已正式发布。此前 `e1a1f45` lint 暴露的非零 base 游标错误已由 `4bba613` 修正，并由 `6341d00` 全量 CI 验证。代码发布完成；现场/部署退出条件仍未满足，不得据此宣称 HA GA 或关闭对应验收工作包。

## 调整后的实施计划

### 第 0 阶段：最新版基线复核与安全收敛

- 实施启动时的对照基线为 `v0.22.0 / 29e0e6c`；该轮代码现已经 PR #1/#2 合并并发布为 `v0.23.0`。后续修复应继续以最新 `main`/正式 tag 对照，不把环境验收状态与代码发布状态混为一谈。
...
退出条件：基线台账逐项指向当时最新版代码；安全正确性缺陷有修复或明确阻断记录；事实投递代码完成审查。上述代码门槛后经 PR #1/#2 合并，`v0.23.0` 发布工作流通过；物理断电、跨主机网络和真实客户端验收仍未完成。
...
- W04 已接通租约 generation 与持久事实事件；HA REQUEST/续租在 lease 与 facts 两个 durable watermarks 确认后 ACK，控制库投影仍异步且可重放；断连、重启、控制写失败恢复和接管后首事件序列连续性有自动化回归。剩余工作是长期控制库故障、旧主回归对账和部署/现场验收。
...
### W04 跨库事实传输阶段记录（2026-09-24；后续已合并发布）
...
- **2026-09-24 更新**：HA protocol v3 已接入 lease/facts 双水位、分块全量重连快照和运行期增量 facts 帧；增量从备端 durable ACK 水位开始，验证连续 sequence、chunk manifest 与 base watermark 后才原子追加。HA primary 默认装配 mapped lease facts producer，REQUEST/续租在 lease 与 facts 双水位均确认后才 ACK；standby 接管遇 facts gap 会 fail closed。自动化已覆盖坏 manifest、非零 base delta 连续性、live delta、delta apply 数据库故障回滚、断开后 stale staging 清理、备端离线追平、主备同时重启以及 control 写入失败后的重试恢复。相关代码后经 PR #1/#2 合并并发布为 `v0.23.0`；控制库长期故障下重放、promoted primary/旧主回归对账及真实双主机 fencing/分区演练仍未验收，故仍不宣称 HA GA。
...
- W01 仓内代码门槛已闭合；还需在真实 secondary 上验证 IXFR TCP 多消息 framing、历史 pruning 边界和互操作恢复。未完成现场验证前，任何缺链仍会返回 AXFR；该阶段记录本身不构成整体 GA 证据；`v0.23.0` 已发布，但真实 secondary 验收仍未完成。
...
- 持续 W06 审计发现 RFC 2136 动态更新对 owner 使用 SQLite 字符串精确比较，可能因输入大小写与既有记录不同而漏删、误判 prerequisite 或生成重复 owner。现在更新操作将新 owner 规范化为小写；匹配、删除、RRset TTL 维护、CNAME 互斥及 prerequisite/audit 查询统一忽略大小写与末尾点，并在控制库和数据面库增加对应表达式索引以避免规范化查询退化为全表扫描。`go build ./...` 与 `go vet ./...` 通过，最新 CI 回归仍需通过。

### W13 DNS 区域导入体验（2026-09-24，代码实现完成）

- DNS 区域详情增加 CSV/BIND 导入入口，可选择本地文件或粘贴内容；先调用 dry-run 展示记录数、类型统计与逐条冲突，再由用户明确点击执行导入。内容或格式变化会清除预检结果；真正导入若发生并发冲突会展示服务端明细并要求重新预检。
- 修复管理端遗留的 multipart 上传与后端 JSON 导入接口不一致问题，统一发送 `{content, format, dry_run}`。中英文文案与 schema 类型已更新，Go 内嵌使用的 `web/dist` 由当前管理端构建产物重新生成。
- `pnpm typecheck` 与 `pnpm build` 通过。IP 地址详情与子网到池向导已在当前代码存在；权限契约全量核对和真实浏览器验收仍未完成。

### W13 建池计划服务端冲突闸门（2026-09-24，代码实现完成）

- 对带有计划指纹的 DHCP scope 创建，服务端此前只拒绝过期指纹，没有拒绝计划本身的阻断冲突；管理端虽隐藏“下一步”，直接调用 API 仍可创建。
- `CheckDHCPScopePlan` 现在先检查指纹是否仍匹配，再拒绝当前计划中的阻断冲突，并通过 HTTP 409 返回结构化原因。没有提交计划引用的手动建池/API 路径保留原有行为。
- IP 地址详情与子网导入/建池向导已在最新版代码存在；本轮补齐 DHCP scope/reservation/option 编辑和 DNS 记录编辑的 `write` 权限门禁及处理器保护。W13 剩余项收敛为权限契约全量核对和真实浏览器验收。
...
- 当前增量完成 `go build ./...`、`pnpm --dir web-admin typecheck`、生产构建和内嵌 `web/dist` 同步。跨模块权限修复另有真实浏览器/API 最小权限验收及最新 PR CI 全绿证据；没有用静态检查替代端到端权限验收。
- 后续复核发现 DNS 客户端执行上游查询的按钮和处理函数缺少 `dns:read` 前置门禁；现已同时禁用无读权限用户的执行按钮，并在处理函数入口再次校验。本轮前端类型检查、生产构建、Go 构建、内嵌资源同步和差异检查通过；最新提交的 PR CI 全量 job 均通过。
- 同轮继续逐处理函数盘点，补上 DNS 区域导入预检/执行对 `dns:write` 的按钮及处理器门禁，以及配置发布队列失败项重试处理器的 `settings:write` 复核；路由仍保留服务端权限校验。该增量的类型检查、构建、内嵌资源同步、浏览器/API 回归和最新 PR CI 均已通过。
- 对所有页面中的表单开启、确认框开启、行操作与内联回调继续扫描后，为 DNS zone/record/DNSSEC/forwarder/blocklist、DHCP scope/reservation/option 创建、租约释放、allowed-list 导入、备份恢复/删除、token 删除、IPAM 地址释放和配置回滚打开动作补齐 `read`/`write`/`delete` 前置检查；用户开关和用户/组/角色/IPAM/DHCP 表格内联编辑也在回调内重新校验。只读会话管理仍由 API 限定用户只能列出/撤销自身会话，管理员撤销他人会话要求 `user:delete`。
- 静态权限审计覆盖全部管理端 view 的动作处理函数、表格内联回调及组件级交互入口；剩余静态扫描命中仅为只读筛选/本地文件选择、内建登录流程和会话自服务（后者有服务端所有权校验）。W13 代码级权限入口核对现已完成；真实浏览器/API 最小权限场景在本地通过，最新提交的 PR CI 全量 job 通过。每个管理对话框的完整鼠标操作路径和目标部署验收仍未逐一覆盖。
- Playwright 最小权限登录场景通过管理员 API 创建临时角色和用户，逐个授予九类资源的单一 `read` 权限；每轮确认该资源 GET 返回 200，其余八类资源 GET 全部返回 403。恢复九类只读权限后，验证 DNS、DHCP、IPAM、备份、系统设置、token、用户、角色、用户组页面可读且写操作入口隐藏。同时确认 DNS 删除、用户删除、角色编辑/授权/删除被禁用，并直接请求九类写 API，均返回 403。新增双角色权限并集场景：DNS 与 DHCP 读取权限分属两个角色，用户只可读取这两类资源，其他七类仍为 403；恢复全量只读权限后复验页面与写入拒绝。新增受限管理员流程：持有 user/role/group read/write 的操作者创建角色并设置 DNS read 权限，创建组并分配角色，再创建用户并分配该组与角色；新用户 DNS 查询获准、DHCP 查询仍拒绝。所有场景在真实本地服务通过，临时用户、角色和区域由用例清理。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，本地真实浏览器/API 场景通过。九类资源页面、九种单资源读取隔离、关键 RBAC 禁止操作、九类 API 写入拒绝、双角色权限并集和受限管理员授权工作流均通过本地复验，`fc5ebff` 阶段的两组 CI 全部通过，后续 PR #1/#2 CI 与 `v0.23.0` 发布工作流也通过；管理端各授权对话框的完整鼠标操作路径未逐一覆盖。

## 范围与限制

本计划按用户授权直接选择工程默认值，已记录在 ADR-0008；DHCP HA 默认沿用已接受的 ADR-0003。实施分支从 `v0.22.0 / 29e0e6c` 创建，相关代码现已合入 `main` 并发布为 `v0.23.0`。目标客户的容量和真实拓扑仍需在现场验证中固定，未有证据前不作数值承诺。
