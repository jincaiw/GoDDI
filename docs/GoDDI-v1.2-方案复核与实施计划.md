...
- Record API 创建/更新、配置发布和 DHCP DDNS 共用 DNS 大小写无关的 CNAME 独占性事务校验；拒绝同一 owner 下不同 CNAME 目标以及 CNAME 与其他类型共存。更新路径会排除当前记录。回归覆盖大小写不同的冲突创建/改名、zonefile 整批回滚、DHCP A 与现有 CNAME 冲突、配置发布与 data-plane CNAME 冲突。`go test ./internal/dns/zone ./internal/dhcp ./internal/configver` 通过。
