...
W12-a/W12-c 隔离式 restore 演练通过；历史实现提交 `8641d20` 和 `bf6d5f9` 的 CI 全通过；W13 真实浏览器/API 权限场景在本地通过。提交 `6341d00` 的远端 CI 全通过。`fa4049a` 包含 HA facts lag 指标及主备重启、control 写失败恢复回归，本地全量 Go tests、metrics/HA/facts/cmd race、vet、golangci-lint v2.13.2 和 diff 检查均通过；其远端 test job 当时仍在运行。当前工作树新增接管后 facts 序列连续性回归并已通过定向测试，完成后需由 CI 复验。此前 `e1a1f45` lint 暴露的非零 base 游标错误已由 `4bba613` 修正，并由 `6341d00` 全量 CI 验证。现场/部署退出条件仍未满足，当前分支保持 draft，未达正式发布门槛。
...
- 同轮继续逐处理函数盘点，补上 DNS 区域导入预检/执行对 `dns:write` 的按钮及处理器门禁，以及配置发布队列失败项重试处理器的 `settings:write` 复核；路由仍保留服务端权限校验。该新增增量待本轮类型检查、构建、内嵌资源同步和 PR CI 验收。
...
- 静态权限审计覆盖全部管理端 view 的动作处理函数、表格内联回调及组件级交互入口；剩余静态扫描命中仅为只读筛选/本地文件选择、内建登录流程和会话自服务（后者有服务端所有权校验）。W13 代码级权限入口核对现已完成，真实浏览器端到端权限验收仍未完成；当前前端类型检查、生产构建、Go 构建与差异检查通过，PR CI 测试 job 尚待通过。
- Playwright 最小权限登录场景通过管理员 API 创建临时角色和用户，逐个授予九类资源的单一 `read` 权限；每轮确认该资源 GET 返回 200，其余八类资源 GET 全部返回 403。恢复九类只读权限后，验证 DNS、DHCP、IPAM、备份、系统设置、token、用户、角色、用户组页面可读且写操作入口隐藏。同时确认 DNS 删除、用户删除、角色编辑/授权/删除被禁用，并直接请求九类写 API，均返回 403。新增双角色权限并集场景：DNS 与 DHCP 读取权限分属两个角色，用户只可读取这两类资源，其他七类仍为 403；恢复全量只读权限后复验页面与写入拒绝。新增受限管理员流程：持有 user/role/group read/write 的操作者创建角色并设置 DNS read 权限，创建组并分配角色，再创建用户并分配该组与角色；新用户 DNS 查询获准、DHCP 查询仍拒绝。所有场景在真实本地服务通过，临时用户、角色和区域由用例清理。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，DNS 场景本地通过。九类资源页面、九种单资源读取隔离、关键 RBAC 禁止操作、九类 API 写入拒绝、双角色权限并集和受限管理员授权工作流的本地真实浏览器/API 复验通过；此前提交 `8641d20` 与 `bf6d5f9` 的全部 CI 检查均通过。最新增量 CI 待通过，管理端各授权对话框的完整鼠标操作路径未逐一覆盖。
