...
| W13 产品体验 | IP 地址详情抽屉、子网导入与建池向导、DNS CSV/BIND 预检导入均已有实现；建池冲突服务端闸门和跨模块详情权限隔离已接通；DNS、DHCP、IPAM、RBAC、备份、系统配置和 token 页面操作入口及处理器已完成静态权限逐项核对；只读角色浏览器回归覆盖九类资源页面、关键 RBAC 按钮及九类 API 写操作拒绝 | 最新 CI 需通过；更细粒度的跨资源授权隔离和管理流程端到端组合仍需补充 |
...
- Playwright 最小权限登录场景通过管理员 API 创建临时只读角色和用户，验证 DNS、DHCP、IPAM、备份、系统配置、token、用户、角色、用户组页面可读且写操作入口隐藏；同时确认 DNS 删除、用户删除、角色编辑/授权/删除被禁用，并直接请求 DNS、DHCP、IPAM、备份、token、系统配置、用户、角色和用户组的九类写 API，均返回 403。用例在 `finally` 中清理临时用户、角色和区域。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，DNS 场景本地通过。场景经两轮扩展后，九类资源页面、关键 RBAC 禁止操作及九类 API 写入拒绝的本地真实浏览器/API 复验通过；更细粒度跨资源授权隔离和本轮最新提交 CI 仍待验收。
