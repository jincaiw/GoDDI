...
- 已新增 Playwright 最小权限登录场景：通过管理员 API 创建仅有 `dns:read` 的临时角色和用户，验证其能打开 DNS 区域页但看不到创建入口、删除动作不可用；用例在 `finally` 中清理临时用户、角色和区域。该项待最新 CI 浏览器套件执行通过后才可记为已验证，其他模块仍需补充权限矩阵场景。
