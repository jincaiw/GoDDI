...
| W13 产品体验 | IP 地址详情抽屉、子网导入与建池向导、DNS CSV/BIND 预检导入均已有实现；建池冲突服务端闸门和跨模块详情权限隔离已接通；DNS、DHCP、IPAM、RBAC、备份、系统配置和 token 页面操作入口及处理器已完成静态权限逐项核对；新增 DNS 只读角色浏览器回归 | 最新 CI 需通过；其余模块仍需按角色补充端到端权限矩阵 |
...
- 已新增 Playwright 最小权限登录场景：通过管理员 API 创建仅有 `dns:read` 的临时角色和用户，验证其能打开 DNS 区域页但看不到创建入口、删除动作不可用；用例在 `finally` 中清理临时用户、角色和区域。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，当前本地真实浏览器场景已通过，最新 CI 全量结果待通过，其他模块仍需补充权限矩阵场景。
