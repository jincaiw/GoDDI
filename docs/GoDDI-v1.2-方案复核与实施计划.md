...
| DHCP→IPAM 持久事件 | 非 HA 默认路径已写入 durable facts outbox 并由 control inbox 原子投影；HA primary 使用 lease/facts 双水位，协议 v3 传输增量 facts，备端校验后事务应用；离线重连、断点 staging 清理、apply 失败回滚、control 写失败恢复、主备重启和接管后首事件连续性均有回归 | 仓内主要实现及本地故障用例已接通；`fc5ebff` 两组远端 CI 全通过；部署/现场验收未完成 | control DB 长期故障重放、旧主回归对账、端到端对账及真实双节点运行态恢复 |
...
| W13 产品体验 | IP 地址详情抽屉、子网导入与建池向导、DNS CSV/BIND 预检导入均已有实现；建池冲突服务端闸门和跨模块详情权限隔离已接通；DNS、DHCP、IPAM、RBAC、备份、系统配置和 token 页面操作入口及处理器已完成静态权限逐项核对；只读 UI 场景覆盖九类资源页面、关键 RBAC 按钮及九类写入拒绝；API 浏览器回归覆盖九类单资源读取隔离、双角色权限并集和受限管理员的 user/role/group 授权工作流；本地真实服务用例通过，`fc5ebff` 远端 CI 全通过 | 授权写入流程覆盖 API 工作流，未覆盖每个管理对话框的完整鼠标操作路径及目标部署验收 |
| W14 容量认证 | 当前无固定硬件/混合负载容量结论 | 固定硬件、负载、故障矩阵和长稳实测；未测数字不对外承诺 |

W12-a/W12-c 隔离式 restore 演练通过；W13 真实浏览器/API 权限场景在本地通过。`fc5ebff` 的本地全量 Go tests、metrics/dataplane/HA/facts/cmd race、vet、golangci-lint v2.13.2、diff 检查及两组远端 test/lint/vulnerability CI 均通过。该提交加入接管后 facts 序列连续性回归。此前 `e1a1f45` lint 暴露的非零 base 游标错误已由 `4bba613` 修正，并由 `6341d00` 全量 CI 验证。PR 可进入代码评审；现场/部署退出条件仍未满足，因此不合并为正式版本、不创建发布标签。
...
- Playwright 最小权限登录场景通过管理员 API 创建临时角色和用户，逐个授予九类资源的单一 `read` 权限；每轮确认该资源 GET 返回 200，其余八类资源 GET 全部返回 403。恢复九类只读权限后，验证 DNS、DHCP、IPAM、备份、系统设置、token、用户、角色、用户组页面可读且写操作入口隐藏。同时确认 DNS 删除、用户删除、角色编辑/授权/删除被禁用，并直接请求九类写 API，均返回 403。新增双角色权限并集场景：DNS 与 DHCP 读取权限分属两个角色，用户只可读取这两类资源，其他七类仍为 403；恢复全量只读权限后复验页面与写入拒绝。新增受限管理员流程：持有 user/role/group read/write 的操作者创建角色并设置 DNS read 权限，创建组并分配角色，再创建用户并分配该组与角色；新用户 DNS 查询获准、DHCP 查询仍拒绝。所有场景在真实本地服务通过，临时用户、角色和区域由用例清理。首轮 CI 暴露角色权限前后端契约不匹配；现保留 POST 单项/批量追加兼容，新增 PUT 完整替换并在单事务中清除/设置权限，角色弹窗切换为 PUT；RBAC 回归覆盖批量幂等、替换、清空及无效输入不破坏旧配置。第二轮 CI 中该浏览器场景超时，测试清理错误遮住了首个失败点；改为独立 API 上下文、固定英文界面并拆分测试步骤后，本地真实浏览器/API 场景通过。九类资源页面、九种单资源读取隔离、关键 RBAC 禁止操作、九类 API 写入拒绝、双角色权限并集和受限管理员授权工作流均通过本地复验，`fc5ebff` 的两组 CI 全部通过；管理端各授权对话框的完整鼠标操作路径未逐一覆盖。
