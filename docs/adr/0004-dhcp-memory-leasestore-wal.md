...
N4.1 已新增 `RecoverMemoryIndex`：先在临时 map 上装载 SQLite/快照基线，再按 sequence replay WAL，全部成功后才原子 `Replace` 目标内存索引；baseline 缺失 ID、WAL gap、损坏或 apply 错误均保持目标索引不变并返回错误。兼容性的 `Replace` API 现复用 `ReplaceChecked`，池快照更新也提供 `SetPoolsChecked`，删除提供 `RemoveChecked`；非法快照或空 lease ID 不会静默发布/删除，恢复路径仍应优先使用可返回错误的 Checked API。该组件固定了启动恢复的 fail-closed 边界，但尚未接入进程启动、控制面快照读取或默认 DHCP 装配。
