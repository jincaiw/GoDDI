...
# 宿主用例要跑三轮、每轮最多等 10s，加上装配与两次完整数据面启动。
...
def slowest_seconds(output):
    """从用例日志里取最慢一轮的秒数，取不到就返回 None。

    grep 型的解析会把「取不到」表现为 0，于是「数字很小」和「根本没量到」变得无法区分。
    """
    match = re.search(r"slowest of \d+ rounds: ([0-9.]+)(m?s|µs|us|ns)", output)
    if not match:
        return None
...
def main():
    ...
    if seconds > 2.1:
        failures.append(f"最慢一轮 {seconds:.3f}s 超过了 2s 界加 0.1s 调度余量")
    ...
    try:
        ...
        if "build failed" in output or "[build failed]" in output:
            ...
            print(output[-2000:])
        elif proc.returncode != 0 and mutated_seconds is not None:
            print(f"变异：撤掉唤醒后最慢一轮 {mutated_seconds:.3f}s，用例失败（{CASE}）")
            ...
            failures.append(
                f"撤掉唤醒后用例仍然通过（最慢一轮 {mutated_seconds}s）：它没有钉住唤醒")
            print(output[-2000:])
    finally:
