...
export const useRouteStore = defineStore(SetupStoreId.Route, () => {
  ...
  function handleConstantAndAuthRoutes() {
    ...
    // Goddi RBAC: drop routes the user has no permission for, so they are
    // neither registered in the router nor shown in menus (direct URL access
    // to a filtered route falls through to not-found → 403).
    const permittedRoutes = filterRoutesByPermission(sortRoutes);

    const vueRoutes = getAuthVueRoutes(permittedRoutes);

    resetVueRoutes();

    addRoutesToVueRouter(vueRoutes);

    getGlobalMenus(permittedRoutes);

    getCacheRoutes(vueRoutes);
  }

  /**
   * Filter routes by Goddi RBAC permission meta
   *
   * A route with `meta.permission` is kept only if the auth store grants it.
   * Parent routes (menu groups) are kept when any child survives.
   *
  ...
  function filterRoutesByPermission(routes: ElegantConstRoute[]): ElegantConstRoute[] {
    ...
    function filterTree(nodes: ElegantConstRoute[]): ElegantConstRoute[] {
      ...
      nodes.forEach(node => {
        ...
        const permission = node.meta?.permission as { resource: string; action: string } | null | undefined;

        const selfAllowed = !permission || authStore.hasPermission(permission.resource, permission.action);
