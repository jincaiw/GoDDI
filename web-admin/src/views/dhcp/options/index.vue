...
import { listDHCPOptions, createDHCPOption, updateDHCPOption, deleteDHCPOption, type DHCPOption, type CreateDHCPOptionRequest } from '@/service/api/goddi/dhcp'
...
const columns = [
  ...
  { title: () => t('common.actions'), key: 'actions', width: 160, render: (row: DHCPOption) => h(NSpace, null, {
    default: () => [
      h(NButton, { size: 'small', text: true, disabled: !perm.canWrite('dhcp'), onClick: () => { if (!perm.canWrite('dhcp')) return; editing.value = row; Object.assign(formData, { code: row.code, value: row.value, priority: row.priority, scope_id: row.scope_id }); showModal.value = true } }, { default: () => t('common.edit') }),
